import resume from "../assets/CV.pdf"

export default function Navbar() {
    return (
    <div className='fixed z-50 bg-dark-500/90 backdrop-blur w-full top-0 left-0 px-8 py-4 lg:px-20 xl:px-36'>
        <div className="flex justify-between items-center text-white">
            <ul className="hidden md:flex">
            <li className="p-4"><a href="#home" className="hover:underline">A propos</a></li>
            <li className="p-4"><a href="#top-projects" className="hover:underline">Projets</a></li>
            <li className="p-4"><a href="#skills" className="hover:underline">Compétences</a></li>
            <li className="p-4"><a href="#contact" className="hover:underline">Me contacter</a></li>
            <li className="p-4"><a href="#certs" className="hover:underline">Timeline</a></li>
            </ul>
            <a href={resume} rel="noreferrer" target="_blank" className="cyber-button rounded-full px-4 py-1">CV</a>
        </div>
    </div>

    )
}

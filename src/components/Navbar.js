  ...
  return (
    <div className="fixed z-50 bg-black w-full top-0 left-0 px-8 py-4 lg:px-20 xl:px-36">
      <div className="flex justify-between items-center text-white">
        ...
        <Link to="/cv" className="cyber-button rounded-full px-4 py-1">CV</Link>
      </div>
      ...
      {menuOpen && (
        <div className="md:hidden fixed top-[72px] left-0 right-0 bg-black py-4 px-8 z-40 border-t border-gray-800">
          <ul className="flex flex-col items-center gap-3">
            {isOnCvPage ? (
              <li>
                <Link to="/" className="hover:underline text-white text-lg" onClick={() => setMenuOpen(false)}>
                  ← Retour
                </Link>
              </li>
            ) : (
              navLinks.map((link) => (
                <li key={link.href}>
                  <a href={link.href} className="hover:underline text-white text-lg" onClick={() => setMenuOpen(false)}>
                    {link.label}
                  </a>
                </li>
              ))
